/**
 * Fund the faucet wallet with LEP100 tokens on Kamet (900523).
 * Sends FUND_AMOUNT (default 10,000) of each token from deployer to faucet wallet.
 * Usage: npx hardhat run scripts/fund-faucet.js --network litho_kamet
 */
...
async function main() {
  ...
  const amount = hre.ethers.utils.parseUnits(FUND_AMOUNT, 18);
  console.log(`Deployer:      ${deployer.address}`);
  console.log(`Faucet wallet: ${FAUCET_WALLET}`);
  console.log(`Amount each:   ${FUND_AMOUNT} tokens\n`);

  for (const t of TOKENS) {
    const contract = new hre.ethers.Contract(t.address, ERC20_ABI, deployer);
    try {
      const tx = await contract.transfer(FAUCET_WALLET, amount, { gasLimit: 100_000 });
      await tx.wait();
      console.log(`✓ ${t.symbol.padEnd(8)} → ${FAUCET_WALLET}  (${tx.hash})`);
    } catch (err) {
