"""Build deterministic POSIX-path Phase 0 R9 Autha handoff archive."""
...
OUT = REPO / "client-work" / "LITHO_PQ_PHASE0_REMEDIATION_R9_2026-08-28.zip"
...
def excluded(source: Path) -> bool:
    ...
    if "__pycache__" in source.parts or source.suffix == ".pyc":
        return True
    if re.fullmatch(r"(?:build|verify)_r[2-8]_package\.py", name):
        return True
    if name.startswith("AUTHA_PHASE0_") and name.endswith("_HANDOFF.md") and name != "AUTHA_PHASE0_R9_HANDOFF.md":
        return True
    if name.startswith("AUTHA_R") and name.endswith("_REMEDIATION_MATRIX.md") and name != "AUTHA_R9_REMEDIATION_MATRIX.md":
        return True
...
def review() -> Path:
    found = list((REPO / "client-work").glob("Autha_Audits_Lithosphere_Makalu_R8_Design_ReReview.docx"))
    if len(found) != 1:
        raise RuntimeError(f"expected one R8 review, found {len(found)}")
    return found[0]
...
def files() -> list[tuple[Path, str]]:
    ...
    selected.append((review(), "review/Autha_PQ_Phase0_R8_Design_ReReview.docx"))
